...
# from models.lsk import *
from models.common import *
from models.attention.HaloAttention import HaloAttention
...
class DecoupledHead1(nn.Module):
    def __init__(self, ch=512, nc=80, width=0.75, anchors=()):
        super().__init__()
        self.nc = nc  # number of classes 类别数
        self.nl = len(anchors)  # number of detection layers 3个输出特征图
        self.na = len(anchors[0]) // 2  # number of anchors 每一个预测特征图有6//2=3个特征框
        self.merge = Conv(ch, ch, 1, 1)
        self.cls_convs = Conv(ch, ch, 3, 1, 1)

        self.reg_convs = Conv(ch, ch, 3, 1, 1)
        ...
        self.theta_preds = nn.Conv2d(ch, 180 * self.na, 1)
    ...
    def forward(self, x):
        ...
        x3 = self.theta_preds(x2)

        # box + obj + cls + theta
        out = torch.cat([x21, x22, x1, x3], 1)
        return out
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        # self.m = nn.ModuleList(DecoupledHead1(x, self.nc, 1, anchors) for x in ch)
        self.mf = nn.ModuleList(nn.Conv2d(x, self.no * self.na, 1) for x in ch)  # output conv
        self.inplace = inplace  # use in-place ops (e.g. slice assignment)
        self.sea = nn.ModuleList(SEAttention(x) for x in ch)
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            xfsea.append((self.sea[i](x[i]))[1])
            
            x[i] = self.m[i](x[i])  # conv
            
            xfsea[i] = self.mf[i](xfsea[i])  # conv
            
            bs, _, ny, nx = x[i].shape  # x[i](bs,self.no * self.na,20,20) to x[i](bs,self.na,20,20,self.no)
            x[i] = x[i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
            
            xfsea[i] = xfsea[i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
        ...
        return (x, xfsea) if self.training else (torch.cat(z, 1), x)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, FGM, AttentionConv, TransformerBlock, ODConv2d, GSConv, C2f]:
            c1, c2 = ch[f], args[0]
