    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                pred, ffpred = model(imgs)  # forward
                # loss, loss_items = compute_loss(pred, targets.to(device), imgs)
                loss, loss_items = compute_loss(pred, targets.to(device), ffpred)  # loss scaled by batch_size
                if RANK != -1:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/ShipRSImageNet.yaml', help='dataset.yaml path')
    ...
    parser.add_argument('--project', default=ROOT / 'debug', help='save to project/name')
