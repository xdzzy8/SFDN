...
from utils.metrics import box_iou, bbox_alpha_iou
...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        Focalcls = FocalLoss(nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['cls_pw']], device=device)), 1.5)
        ...
        self.Focalcls = Focalcls
        self.BCEtheta = BCEtheta
        for k in 'na', 'nc', 'nl', 'anchors':
            setattr(self, k, getattr(det, k))

    def __call__(self, p, targets, ffpred=None):  # predictions, targets, model
        """
        Args:
        ...
        lcls, lbox, lobj, lclsf, lclsff = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if ffpred != None:
                    # pf = fpred[i][b, a, gj, gi]
                    pff = ffpred[i][b, a, gj, gi]
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    ...
                    if ffpred != None:
                        # lclsf += self.BCEcls(pf[:, 5:class_index], t)
                        lclsff += self.BCEcls(ps[:, 5:class_index], t) + 0.1*self.BCEcls(pff[:, 5:class_index], t)  
        ...
        # totallcls = 1.5*lcls + 0.05*lclsf + 0.05*lclsff
        lcls = lcls + lclsff
        # print(lcls.data)
        # print(lclsff.data)
        lbox *= self.hyp['box']
